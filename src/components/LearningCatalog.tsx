...
import { BatchActionBar, BatchManageButton, useListBatch, type BatchAction } from './ListBatch';

/** Complete browsing surface; features own data and render each row. */
export function LearningCatalog<T>({ title, items, searchText, renderRow, locale = 'zh-CN', tools, notice, columns, columnLabels, hasActions, onBack, batch: batchConfig }: {
  ...
  /** Adds a bulk-edit mode; selection covers every row matching the current search. */
  batch?: { id: (item: T) => string; actions: BatchAction[] };
}) {
  ...
  const batch = useListBatch(batchConfig ? filtered.map(batchConfig.id) : []);
  const renderSelectable = (item: T) => {
  ...
  return <LearningListFrame label={title} className="learning-catalog">
    ...
    <LearningList columns={columns} locale={locale} columnLabels={columnLabels} hasActions={hasActions} selection={batch.selection}>{filtered.slice(mobile.mobile ? 0 : current * 8, mobile.mobile ? mobile.visible : current * 8 + 8).map(renderSelectable)}</LearningList>
