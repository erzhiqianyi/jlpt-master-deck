...
export function LearningCatalog<T>({ title, items, searchText, renderRow, locale = 'zh-CN', tools, notice, onBack }: {
  title: string; items: T[]; searchText: (item: T) => string; renderRow: (item: T) => ReactNode;
  locale?: string; tools?: ReactNode; notice?: ReactNode; onBack?: () => void;
}) {
  ...
  return <LearningListFrame label={title} className="learning-catalog">
    ...
    {filtered.length ? <LearningList>{filtered.slice(mobile.mobile ? 0 : current * 8, mobile.mobile ? mobile.visible : current * 8 + 8).map(renderRow)}</LearningList> : <p className="catalog-empty" role="status">{ja ? '該当する項目がありません。' : en ? 'No matching items.' : '没有匹配的内容。'}</p>}
    {mobile.mobile ? <div ref={mobile.setSentinel} className="catalog-notice" role="status">{mobile.visible >= filtered.length ? (ja ? 'すべて表示しました' : en ? 'End of list' : '已经到底了') : null}</div> : null}
